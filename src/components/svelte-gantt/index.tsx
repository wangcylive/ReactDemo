...
const Tree = () => {
  ...
  useEffect(() => {
    if (!loaded) {
      // Create Gantt
      const a = new SvelteGantt({target: document.getElementById('example-gantt'), props: options})
      console.log('a', a)
      a.api.tasks.on.select(task => console.log('Listener: task selected', task))
      setLoaded(true)
    }
  }, [loaded])
  ...
  useEffect(() => {
    if (loaded) {
      const els = document.querySelectorAll('.sg-task')
      ...
      let keys: string[] = [] // 去重复
      els.forEach(node => {
        const matches = node.className.match(bgReg)
        if (matches) {
          matches.forEach(match => {
            ...
            keys.push(match)
            const backgroundColor = match.split('_').pop()
            cssArr.push(`.${match},.${match}:hover,.${match}.selected{background-color:#${backgroundColor}}`)
          })
        }
      })
